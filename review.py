...
if NOTION_API_KEY:
    ...
    notion_page_id = os.getenv("NOTION_PAGE_ID", "")  # Page ID par défaut (optionnel)
...
def get_file_tree(owner, repo, path='', level=0):
    ...
    headers = {'Authorization': f'token {GITHUB_SECRET_KEY}'}
...
class Tools:
    ...
    def get_file_contents(path, owner, repo):
        ...
        # Ajout de l'en-tête Authorization avec le token
        headers = {'Authorization': f'token {GITHUB_SECRET_KEY}', 'X-GitHub-Api-Version': '2022-11-28'}
...
class ReviewCrew:
    ...
    def run(self):
        ...
        # Tâches
        content_task = Tasks.get_file_content_task(agent=content_agent, owner=self.owner, repo=self.repo, path=self.path)
        review_task = Tasks.review_task(agent=review_agent, repo=self.repo, context=[content_task])
        
        if notion_agent and self.page_id:
            notion_task = Tasks.notion_task(agent=notion_agent, page_id=self.page_id, context=[review_task])
...
def main():
    ...
    if not GITHUB_SECRET_KEY:
        print("❌ Clé API GitHub non trouvée. Veuillez configurer la variable d'environnement GITHUB_SECRET_KEY.")
        return
    
    # Saisie utilisateur
    github_url = input("URL du dépôt GitHub à analyser: ")
